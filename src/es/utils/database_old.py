...
class ArtikelnummernLieferanten(Base):
    ...
    __table_args__ = (
        PrimaryKeyConstraint(
            "ArtikelNummer", name="Artikelnummern_Lieferanten$PrimaryKey"
        ),
    ...
    Bild: Mapped[Optional[str]] = mapped_column(
        String(8000, "SQL_Latin1_General_CP1_CI_AS")
    )
...
class EntnahmeArtikel(Base):
    ...
    Anzahl: Mapped[Optional[float]] = mapped_column(
        Float(53), server_default=text("((0))")
    )
...
class KundenÜbersicht(Base):
    ...
    Rechnung_Kunde: Mapped[List["RechnungKunde"]] = relationship(
        "RechnungKunde", back_populates="Kunden_Übersicht"
    )
...
class LieferantES(Base):
    ...
    Bewertung: Mapped[Optional[int]] = mapped_column(
...
class RechnungLieferant(Base):
    ...
    BetragBrutto: Mapped[Optional[Any]] = mapped_column(
        MONEY, server_default=text("((0))")
    )
    ...
    RechnungsbelegDa: Mapped[Optional[bool]] = mapped_column(
        Boolean, server_default=text("((0))")
    )
    ZahlungsbelegDa: Mapped[Optional[bool]] = mapped_column(
        Boolean, server_default=text("((0))")
    )
    Link: Mapped[Optional[str]] = mapped_column(Unicode(255))
...
class RechnungKunde(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["KundenNummer"],
            ["Kunden_Übersicht.KundenNummer"],
            name="FK_4_Rechnung_Kunde_Kunden_Übersicht",
        ),
    ...
    Kunden_Übersicht: Mapped["KundenÜbersicht"] = relationship(
        "KundenÜbersicht", back_populates="Rechnung_Kunde"
    )
